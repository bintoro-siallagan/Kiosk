...
function setupPurchaseReturn(app, opts = {}) {
  ...
  router.patch('/:id', (req, res) => {
    const row = db.prepare(`SELECT * FROM purchase_return_docs WHERE id = ?`).get(req.params.id);
    if (!row) return res.status(404).json({ error: 'tidak ditemukan' });
    const b = req.body || {};
    const fields = [], args = [];
    for (const k of ['return_no', 'supplier', 'po_ref', 'items', 'total_value', 'reason', 'status', 'created_by', 'completed_at']) {
      if (b[k] !== undefined) {
        fields.push(`${k} = ?`);
    ...
    db.prepare(`UPDATE purchase_return_docs SET ${fields.join(', ')} WHERE id = ?`).run(...args);
    res.json({ ok: true });
  });

  router.delete('/:id', (req, res) => {
    const info = db.prepare(`DELETE FROM purchase_return_docs WHERE id = ?`).run(req.params.id);
