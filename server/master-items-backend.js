...
function setupMasterItems(app, opts = {}) {
  ...
  // ─── BULK CSV IMPORT (must precede /menus/:id route) ──────────
  // GET /menus/bulk-template — download CSV template
  ...
  // POST /menus/bulk-csv — upload CSV file, parse, INSERT OR REPLACE
      ...
      const tx = db.transaction(() => {
        for (let i = 1; i < lines.length; i++) {
          ...
          try {
            ...
            const id = get('id') || `${category_id}-${name.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 30)}-${Date.now().toString(36).slice(-4)}-${i}`;
