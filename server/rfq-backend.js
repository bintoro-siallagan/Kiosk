...
function setupRfq(app, opts = {}) {
  ...
  router.patch('/:id', (req, res) => {
    const row = db.prepare(`SELECT * FROM rfq_docs WHERE id = ?`).get(req.params.id);
    if (!row) return res.status(404).json({ error: 'tidak ditemukan' });
    const b = req.body || {};
    const fields = [], args = [];
    for (const k of ['rfq_no', 'item', 'qty', 'unit', 'status', 'awarded_vendor']) {
      if (b[k] !== undefined) { fields.push(`${k} = ?`); args.push(b[k]); }
    }
    if (!fields.length) return res.json({ ok: true, noop: true });
    args.push(req.params.id);
    db.prepare(`UPDATE rfq_docs SET ${fields.join(', ')} WHERE id = ?`).run(...args);
    res.json({ ok: true });
  });

  router.delete('/:id', (req, res) => {
    const info = db.prepare(`DELETE FROM rfq_docs WHERE id = ?`).run(req.params.id);
