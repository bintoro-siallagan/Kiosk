...
function setupIncidents(app, opts = {}) {
  ...
  router.patch('/:id', (req, res) => {
    ...
    const b = req.body || {};
    const fields = [], args = [];
    for (const k of ['incident_no', 'title', 'category', 'outlet', 'severity', 'status', 'reported_by', 'description', 'resolution']) {
      if (b[k] !== undefined) { fields.push(`${k} = ?`); args.push(b[k]); }
    }
    if (!fields.length) return res.json({ ok: true, noop: true });
    args.push(req.params.id);
    db.prepare(`UPDATE incidents SET ${fields.join(', ')} WHERE id = ?`).run(...args);
    res.json({ ok: true });
  });
