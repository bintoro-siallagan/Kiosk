...
function setupSimplePurchase(app, opts = {}) {
  ...
  router.patch('/:id', (req, res) => {
    const row = db.prepare(`SELECT * FROM simple_purchases WHERE id = ?`).get(req.params.id);
    if (!row) return res.status(404).json({ error: 'tidak ditemukan' });
    const b = req.body || {};
    const fields = [], args = [];
    for (const k of ['item_name', 'sku', 'qty', 'unit', 'unit_price', 'vendor', 'payment_method', 'outlet', 'purchased_by', 'notes']) {
      if (b[k] !== undefined) {
        if (k === 'payment_method' && !PAYMENTS.includes(b[k])) continue;
        fields.push(`${k} = ?`);
        args.push(b[k]);
      }
    }
    // recompute total if qty/unit_price changed
    const newQty = b.qty !== undefined ? Number(b.qty) : row.qty;
    const newPrice = b.unit_price !== undefined ? Number(b.unit_price) : row.unit_price;
    if (b.qty !== undefined || b.unit_price !== undefined) {
      fields.push(`total = ?`);
      args.push(Math.round(newQty * newPrice));
    }
    if (!fields.length) return res.json({ ok: true, noop: true });
    args.push(req.params.id);
    db.prepare(`UPDATE simple_purchases SET ${fields.join(', ')} WHERE id = ?`).run(...args);
    res.json({ ok: true });
  });

  router.delete('/:id', (req, res) => {
    const info = db.prepare(`DELETE FROM simple_purchases WHERE id = ?`).run(req.params.id);
