...
export default function AdminAssetMaintenance({ apiBase = "" }) {
  ...
  const remove = async (item) => {
    const ok = await confirm({ title: `Hapus "${item.name || item.asset_code || '#' + item.id}"?`, message: "Akan dihapus permanen. Tidak bisa dibatalkan.", danger: true, okLabel: "Hapus" });
    if (!ok) return;
              ...
              <tr key={a.id} style={{ borderTop: "1px solid #161b22", fontSize: 12 }}>
                ...
                <td style={S.td}>
                  <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => remove(a)} title="Hapus" style={{ background: "#ef444418", border: "1px solid #ef444444", color: "#ef4444", padding: "3px 7px", borderRadius: 5, fontSize: 11, cursor: "pointer", fontFamily: "inherit", fontWeight: 700 }}>🗑️</button>
                  </div>
                </td>
              </tr>
            ))}
