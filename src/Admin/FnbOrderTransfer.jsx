...
export default function FnbOrderTransfer({ apiBase = "" }) {
  ...
  const saveEdit = async () => {
    const r = await fetch(`${base}/order-transfers/${editing.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editing) });
      ...
      {editing && (
        <div onClick={() => setEditing(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#0d1117", border: "1px solid #30363d", borderRadius: 12, padding: 22, maxWidth: 500, width: "100%" }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: "#fff", marginBottom: 14 }}>✏️ Edit Transfer — #{editing.order_id}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <Field label="Order ref"><input value={editing.order_ref || ""} onChange={e => setEditing({ ...editing, order_ref: e.target.value })} style={inp} /></Field>
              <Field label="Oleh"><input value={editing.transferred_by || ""} onChange={e => setEditing({ ...editing, transferred_by: e.target.value })} style={inp} /></Field>
              <Field label="Dari meja"><input value={editing.from_table || ""} onChange={e => setEditing({ ...editing, from_table: e.target.value })} style={inp} /></Field>
              <Field label="Ke meja"><input value={editing.to_table || ""} onChange={e => setEditing({ ...editing, to_table: e.target.value })} style={inp} /></Field>
            </div>
...
const Ba = (color) => ({ background: color + "18", border: `1px solid ${color}44`, color, padding: "3px 7px", borderRadius: 5, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" });
