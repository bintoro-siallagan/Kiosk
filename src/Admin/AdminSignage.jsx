          ...
          {d.screens.map(sc => {
            ...
            return (
              <div key={sc.id} onClick={() => post(`screen/${sc.id}/toggle`, null, `✓ ${sc.outlet} ${on ? "offline" : "online"}`)}
                ...
                <div style={{ fontSize: 9, fontWeight: 700, color: on ? "#10b981" : "#5b6470", fontFamily: "'Space Mono',monospace", marginTop: 6 }}>
                  {on ? "● ONLINE" : "○ OFFLINE"}
                </div>
              </div>
            );
      ...
      <div style={{ ...S.card, marginTop: 14 }}>
        ...
        {d.media.map(m => (
          <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "8px 0", borderTop: "1px solid #161b22", fontSize: 12 }}>
            ...
            <span style={{ width: 50, fontFamily: "'Space Mono',monospace", color: "#9da7b3" }}>{m.duration_sec}s</span>
            <button onClick={() => post(`media/${m.id}/toggle`, null, `✓ ${m.title} di-toggle`)}
              style={{ width: 90, fontSize: 10, fontWeight: 700, color: MST[m.status], background: MST[m.status] + "1f", border: `1px solid ${MST[m.status]}55`, borderRadius: 5, padding: "3px 8px", fontFamily: "'Space Mono',monospace", cursor: "pointer" }}>
