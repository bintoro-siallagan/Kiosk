...
const STEPS = [
  ...
  {key:"completed", icon:"🏁", label:"Selesai",           desc:"Terima kasih telah memesan di BINTORO"},
];
      ...
      <div style={T.body}>
        ...
        {!urlOrderId && (
          <div style={{...T.card,marginBottom:20,textAlign:"center"}}>
            ...
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:16,fontWeight:700,marginBottom:6}}>Cek Status Pesanan</div>
        ...
        {!loading && error && urlOrderId && (
          <div style={{...T.card,textAlign:"center"}}>
            ...
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:16,fontWeight:700,marginBottom:6}}>Pesanan Tidak Ditemukan</div>
          ...
          <div style={{animation:"fadeUp 0.3s ease"}}>
            ...
            <div style={{...T.card,marginBottom:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                <div>
                  <div style={{fontFamily:"'Space Mono',monospace",fontSize:24,fontWeight:700,color:"#F59E0B",letterSpacing:2}}>
                    #{order.id}
                  </div>
                ...
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:"'Space Mono',monospace",fontSize:18,fontWeight:700,color:"#fff"}}>{fIDR(order.total)}</div>
              ...
              <div style={{borderTop:"1px solid #1a1a2e",paddingTop:12}}>
                {(order.items||[]).map((item,i)=>(
                  <div key={i} style={{display:"flex",gap:10,marginBottom:8,alignItems:"center"}}>
                    ...
                    <span style={{fontSize:13,fontWeight:600,fontFamily:"'Space Mono',monospace",color:"#F59E0B"}}>{fIDR(item.p*item.q)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {isCancelled ? (
              <div style={{...T.card,textAlign:"center",borderColor:"#F8717133"}}>
                <div style={{fontSize:48,marginBottom:12}}>❌</div>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:18,fontWeight:700,color:"#F87171",marginBottom:6}}>PESANAN DIBATALKAN</div>
                <div style={{fontSize:13,color:"#666"}}>Pesanan ini telah dibatalkan. Silakan hubungi kasir.</div>
              </div>
            ) : (
              <div style={{...T.card,marginBottom:16}}>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:11,fontWeight:700,color:"#555",letterSpacing:2,marginBottom:20}}>STATUS PESANAN</div>
                ...
                {!["completed","cancelled"].includes(order.status) && (
                  <div style={{background:"rgba(245,158,11,0.05)",border:"1px solid rgba(245,158,11,0.15)",borderRadius:12,padding:"12px 16px",marginTop:16,display:"flex",alignItems:"center",gap:12}}>
                    ...
                    <div style={{marginLeft:"auto",fontFamily:"'Space Mono',monospace",fontSize:11,color:"#555"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button style={T.waBtn} onClick={()=>{
                ...
                const msg = encodeURIComponent(`Cek status pesanan BINTORO #${order.id} saya di sini: ${url}`);
...
const T = {
  root:   {fontFamily:"'Plus Jakarta Sans',sans-serif",background:"#050810",color:"#fff",minHeight:"100vh"},
  ...
  logo:   {fontSize:28},
  brand:  {fontFamily:"'Space Mono',monospace",fontSize:16,fontWeight:700,color:"#F59E0B",letterSpacing:2},
  ...
  searchInput:{flex:1,background:"#080c10",border:"1px solid #21262d",borderRadius:10,padding:"10px 14px",color:"#fff",fontSize:16,fontFamily:"'Space Mono',monospace",letterSpacing:2,textTransform:"uppercase"},
  searchBtn:{background:"linear-gradient(90deg,#F59E0B,#F97316)",border:"none",borderRadius:10,padding:"10px 20px",color:"#050810",fontWeight:700,fontSize:13,letterSpacing:1,fontFamily:"'Space Mono',monospace"},
