...
export default function CommandCustomer() {
  ...
  const load = useCallback(() => {
    fetch(`${API}/api/section/customer`).then(r => r.json()).then(j => j && !j.error ? setD(j) : setErr((j && j.error) || "data tidak tersedia")).catch(e => setErr(String(e)));
  }, []);
...
const S = {
  ...
  chCard: { background: "#080a0f", border: "1px solid #21262d", borderRadius: 10, padding: "12px 14px" },
  lbRow: { display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: "1px solid #161b22" },
};
